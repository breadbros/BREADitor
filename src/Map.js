...
export function Map(mapfile, mapdatafile, updateLocationFunction) {
    ...
    this.getLayerByIdx_DANGEROUS = idx => {
        ...
        throw new Error(
            `Invalid layer index "${idx}".  Valid range [0, ${this.layers.length - 1}]`
        );
    };
    ...
    this.createEntityRenderData = () => {
        ...
        for (const i in this.entities) {
            if (this.entities[i]) {
                INFO(
                    'Sorting entities on layer',
                    i,
                    ', ',
                    this.entities[i].length,
                    'entities to sort'
                );
...
Map.prototype = {
    sullyDataHealing(entity) {
        ...
        if (!this.entityData[entity.filename]) {
            ...
            try {
                ...
                window.alert(
                    `Failure while attempting to parse json for ${datafile}\nReason: \n${e}`
                );
            ...
            if (data) {
                ...
                console.warn(
                    `Could not find '${entity.filename}', using the default. Path: `,
                    datafile
                );
                // debugger;
    ...
    addEntityWithoutSort(entity, location) {
        ...
        if (!entity.filename.endsWith('chr') && !entity.filename.endsWith('json')) {
            console.warn(
                `entity ('${entity.filename}') has an unknown format.  Skipping for now. (2)`
            );
            entity.MAPED_USEDEFAULT = true;
            return;
        }
    ...
    maybeRenderZones(gl) {
        ...
        if (getZoneVisibility() && this.zoneData.length > 1) {
            ...
            gl.uniform4f(
                ...
                this.renderContainerDimensions.w /
                    this.vspData[vsp].tilesize.width /
                    this.camera[2],
                this.renderContainerDimensions.h /
                    this.vspData[vsp].tilesize.height /
                    this.camera[2]
            );
    ...
    maybeRenderMarchingAnts(gl, selection) {
        ...
        if (selection.lines.length > 0 && typeof vsp !== 'undefined') {
            ...
            const appliedOffset = selection.map.mapData.isTileSelectorMap
                ? { X: 0, Y: 0 }
                : layer.offset; // If tileset selector, ignore offset
    ...
    */
    ...
    renderEntity(entity, layer, tint, clip, mask, isTallRedraw) {
        ...
        if (
            ...
            this.renderEntityTallRedrawBoundsBounds(
                entity,
                layer,
                tint,
                clip,
